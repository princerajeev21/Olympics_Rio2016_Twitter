function(input, output, session) {
  ...
  data_retweet<-reactive({
    
    getRetweets(input$selection)
  ...
  output$score1<-renderPlot({
    ...
    data<-data_emotion()
    ggplot(data,aes(x=score))+geom_histogram(bins=50,color="black",fill="red")+theme_minimal()+xlab("Sentiment Score")+ylab("count")+ggtitle("Sentiment Scores of reviews")
  })
